...
const SearchedUser = ({route, navigation}: Props) => {
  const {userName, userPostDetail}: any = useContext(UserContext);

  const [isModelOpen, setModelOpen] = useState(false);

  const {searchedValue} = route.params;

  const searchedPostDetail = userPostDetail.find(
    (element: {title: any}) => element.title === searchedValue,
  );

  const displayName = () => {
    const user = userName.find(
      (element: {id: any}) => searchedPostDetail.userId === element.id,
    );
    return (
      <View>
    ...
    return (
      <View>
        <Modal
          ...
          <TouchableWithoutFeedback onPress={() => setModelOpen(false)}>
          ...
          <View style={styles.popup}>
            ...
            <Text style={styles.modalText as any}>
  ...
  return (
    <>
      <View style={[styles.pop, isModelOpen ? {opacity: 0.5} : '']}>
        <View>
          {ClickMe()}
          {ModalView()}
          <Text style={styles.searchedResult}>
            The result you selected is : {`"${searchedValue}"`}
          </Text>
          {displayName()}
          {displayPostData()}
        </View>
