...
const Search = ({route, navigation}: Props) => {
  const {name, postDetailObj}: any = useContext(UserContext);

  const [ModelVal, setModelVal] = useState(false);

  const gettingUserData = name.map((el: Object) => {
    return el;
  });

  const gettingPostData = postDetailObj.map((el: Object) => {
    return el;
  });
  ...
  const searchedPostDetail = gettingPostData.find(
    (el: {title: any}) => el.title === element,
  );

  const displayName = () => {
    const user = gettingUserData.find(
      (el: {id: any}) => searchedPostDetail.userId === el.id,
    );
    return (
      <View>
